...
- Python 파일, 모듈, 함수, 변수 이름은 기본적으로 `snake_case`를 사용한다.
- 클래스와 예외 타입 이름은 `PascalCase`를 사용한다.
- 상수 이름은 `UPPER_SNAKE_CASE`를 사용한다.
- 비동기 함수도 일반 함수와 같은 네이밍 규칙을 따르되, 역할이 드러나는 동사를 이름에 포함한다.
- RESP 명령어 문자열은 입력 시 대소문자와 공백 차이를 허용하더라도 내부 처리 직전에는 대문자 기준으로 정규화한다.
- 공용 인터페이스에서는 의미가 겹치는 이름을 피하고 `command`, `tokens`, `key`, `value`, `response`, `reader`, `writer`처럼 역할이 분명한 이름을 우선 사용한다.
- 팀 공용 계약에서 합의한 이름은 담당 모듈마다 임의로 바꾸지 않는다.
